<?php
$mid = isset( $_GET['modno']) ? $_GET['modno'] : ''  ;
     $qr=$data->getSelect("select s.snm,(select id from sw.srv where snm=s.snm limit 1) as sid from sw.screen s where mod=$mid and (select id from sw.srv where snm=s.snm limit 1) is not null", $rA, 'S', __LINE__ );
      while (! ($qr->EOF))
          {
          $sid=$qr->f['sid'];
